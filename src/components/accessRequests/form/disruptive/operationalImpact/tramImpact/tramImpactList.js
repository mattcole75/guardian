...
const TramImpactList = (props) => {
    ...
    const { id, save, roles, tramImpactItems, status } = props;

    // const { tramServiceDisruptionItems, disruptiveStatus } = disruptive;
    ...
    const addTramImpactItem = () => {
        ...
        if(tramImpactItems != null) {
            updatedTramImpactItems = [ ...tramImpactItems ];
        }
        // add new item
        ...
        save(id, { tramImpactItems: updatedTramImpactItems });
    };

    const deleteTramImpactItem = (index) => {
        // create local copy
        const updatedTramImpactItems = [ ...tramImpactItems ];
            ...
            { tramImpactItems
                ?   <div className='table-responsive'>
                        <table className='w-100 table table-hover table-borderless align-middle bg-light border-start border-end border-top shadow-sm fs-7'>
                            ...
                            <tbody>
                                {
                                    tramImpactItems.map((item, index) => {
